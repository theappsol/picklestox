	...
	control: {
		...
		'cryptoairdrop-color': {
			...
			init: function ( control ) {
				...
				// Init the control.
				cryptoairdrop.input.color.init( control );
			},
			...
			template: function ( control ) {
				control.container.html(
					cryptoairdrop.input.color.getTemplate( {
		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="cryptoairdrop-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="cryptoairdrop-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.cryptoairdrop-color-control[data-id="' + control.id + '"]'
				    ),
				    clear;
				...
				setTimeout( function () {
					clear = jQuery(
						'.cryptoairdrop-input-container[data-id="' +
						control.id +
						'"] .wp-picker-clear'
					);
					if ( clear.length ) {
...
(
	function () {
		 ...
		 */
		wp.customize.cryptoairdropDynamicControl = wp.customize.Control.extend( {
			initialize: function ( id, options ) {
				...
				if ( !args.params.type ) {
					args.params.type = 'cryptoairdrop-generic';
				}
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initcryptoairdropControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'cryptoairdrop-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initcryptoairdropControl: function ( control ) {
				if ( 'undefined' !== typeof cryptoairdrop.control[control.params.type] ) {
					cryptoairdrop.control[control.params.type].init( control );
					return;
				}
			...
			cryptoairdropValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( cryptoairdrop.control, function ( obj, type ) {
	wp.customize.controlConstructor[
		type
		] = wp.customize.cryptoairdropDynamicControl.extend( {} );
} );
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['cryptoairdrop-slider'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		var control      = this,
		    ...
		    slider       = control.container.find( '.cryptoairdrop-slider' ),
		...
		slider.slider( {
			...
			change: function ( event, ui ) {
				...
				// Save the values.
				control.initcryptoairdropControl();
			}
		} );
		...
		// Init the control.
		if ( !_.isUndefined( window.cryptoairdropControlLoader ) && _.isFunction( cryptoairdropControlLoader ) ) {
			cryptoairdropControlLoader( control );
		} else {
			control.initcryptoairdropControl();
		}
	},

	initcryptoairdropControl: function () {
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['cryptoairdrop-text'] = wp.customize.Control.extend( {
 ...
 * Control: Sortable.
 */
/* global cryptoairdropControlLoader */
wp.customize.controlConstructor['cryptoairdrop-sortable'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		// Init the control.
		if (
			!_.isUndefined( window.cryptoairdropControlLoader ) &&
			_.isFunction( cryptoairdropControlLoader )
		) {
			cryptoairdropControlLoader( control );
		} else {
			control.initcryptoairdropControl();
	...
	initcryptoairdropControl: function () {
		...
		var control = this;
		control.container.find( '.cryptoairdrop-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['cryptoairdrop-radio-image'] = wp.customize.cryptoairdropDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['cryptoairdrop-toggle'] = wp.customize.cryptoairdropDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['cryptoairdrop-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
;
/* global dimensionscryptoairdropL10n */
/* global cryptoairdropControlLoader */
wp.customize.controlConstructor['cryptoairdrop-dimensions'] = wp.customize.cryptoairdropDynamicControl.extend( {
	...
	updateDimensionsValue: function ( context, value ) {
		...
		control.container.on( 'change keyup paste', '.' + context + ' input', function () {
			...
			// Notifications.
			control.cryptoairdropNotifications();
	 ...
	 */
	cryptoairdropNotifications: function () {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.cryptoairdropValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionscryptoairdropL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionscryptoairdropL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global cryptoairdropControlLoader */
wp.customize.controlConstructor['cryptoairdrop-background'] = wp.customize.Control.extend( {
	...
	initcryptoairdropControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.cryptoairdrop-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['cryptoairdrop-editor'] = wp.customize.cryptoairdropDynamicControl.extend( {
	...
	initcryptoairdropControl: function () {
		...
		var id = 'cryptoairdrop-editor-' + control_id;
